...
(SELECT AS struct payload.* replace (cast(version as float64) as version)) AS payload,
null as sample_id,
  TIMESTAMP_MICROS(DIV(metadata.timestamp, 1000)) AS submission_timestamp  ,
type,
version
from `moz-fx-data-derived-datasets.telemetry_derived.telemetry_heartbeat_parquet_v1`  
